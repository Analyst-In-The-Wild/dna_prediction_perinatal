...
contacts <- get_rio_contacts(select_vars = FALSE) %>% 
    ...
    mutate(contact_month = as.Date(lubridate::floor_date(contacts_contact_datetime, unit = "month")))
...
current_pats <- get_rio_referrals() %>% 
    ...
    mutate(patient_name = paste(forename, surname, sep = " "),
        ...
        referrals_referral_datetime = as.Date(lubridate::floor_date(referrals_referral_datetime, unit = "month")))

current_pats_risk <- current_pats %>% 
    mutate(patient_name = "PATIENT NAME",
...
current_pats_caseload <- get_rio_referrals() %>% 
    ...
    mutate(patient_name = paste(forename, surname, sep = " "),
        referral_age  = eeptools::age_calc(as.Date(date_of_birth), as.Date(referrals_referral_datetime), units = "years", precise = FALSE),
        referrals_referral_datetime = as.Date(lubridate::floor_date(referrals_referral_datetime, unit = "month")),
        next_booked_appt = as.Date(lubridate::floor_date(next_booked_appt, unit = "month")))
...
current_pats_na_ethnic <- current_pats_caseload %>% 
    left_join(next_appt) %>% 
    # left_join(hcp_names) %>% 
    mutate(referrals_referral_datetime = as.Date(lubridate::floor_date(referrals_referral_datetime, unit = "week")),
           referrals_firstappt_datetime = as.Date(lubridate::floor_date(referrals_firstappt_datetime, unit = "week")),
           hcp_name = "HCP NAME",
           patient_name = "PATIENT NAME",
           client_id = "PATIENT IDENTIFIER",
           contacts_contact_datetime = as.Date(lubridate::floor_date(contacts_contact_datetime, unit = "week"))) %>% 
